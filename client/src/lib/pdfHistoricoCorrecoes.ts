/**
 * Gerador de PDF — Módulo 5: Histórico de Correções
...
export interface PdfHistoricoCorrecoesData {
  kpis: { creditInicial: number; creditFinal: number; correcaoAcumulada: number; totalPago: number };
  yearlyTable: { year: string; creditEndOfYear: string; balanceEndOfYear: string; correctionInYear: string; correctionAccum: string; paidInYear: string; avgInstallment: string; events: string }[];
  warnings: string[];
  simulationId: string;
  generatedAt: string;
...
function drawFooter(doc: jsPDF, data: PdfHistoricoCorrecoesData) {
...
export async function generatePdfHistoricoCorrecoes(data: PdfHistoricoCorrecoesData): Promise<void> {
  ...
  doc.text("Raio-X do Consórcio · Módulo 5: Histórico de Correções", 14, 26);
  ...
  autoTable(doc, {
    ...
    body: [
      ["Carta inicial", brl0(data.kpis.creditInicial)],
      ["Carta final projetada", brl0(data.kpis.creditFinal)],
      ["Correção acumulada", brl0(data.kpis.correcaoAcumulada)],
      ["Total pago projetado", brl0(data.kpis.totalPago)],
    ],
    headStyles: { fillColor: INK, textColor: WHITE, fontStyle: "bold", fontSize: 8 },
  ...
  doc.addPage(); y = 20;
  doc.setFontSize(10); doc.setFont("helvetica", "bold"); doc.setTextColor(...INK);
  doc.text("Tabela anual de correções", 14, y); y += 6;
  autoTable(doc, {
    startY: y,
    head: [["Ano", "Carta final", "Saldo final", "Correção no ano", "Correção acum.", "Pago no ano", "Parcela média", "Eventos"]],
    body: data.yearlyTable.map((r) => [r.year, r.creditEndOfYear, r.balanceEndOfYear, r.correctionInYear, r.correctionAccum, r.paidInYear, r.avgInstallment, r.events]),
    headStyles: { fillColor: INK, textColor: WHITE, fontStyle: "bold", fontSize: 6.5 },
  ...
  drawFooter(doc, data);
  doc.save(`raio-x-historico-correcoes-${data.simulationId}.pdf`);
}
