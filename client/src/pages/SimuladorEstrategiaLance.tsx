    ...
    <form onSubmit={(e) => { e.preventDefault(); handleCalculate(); }} className="space-y-1.5">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5">
        <div className="flex flex-col h-full">
          <label className="block text-[14px] md:text-[13px] font-bold text-gray-800 mb-0.5 truncate">Carta (R$)</label>
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="mt-auto">
            ...
            <p className="text-[12px] text-foreground/40 mt-0.5 leading-tight truncate">Sobre carta</p>
          </div>
        </div>
        <div className="flex flex-col h-full">
          <label className="block text-[14px] md:text-[13px] font-bold text-gray-800 mb-0.5 truncate">Lance (%)</label>
          <div className="mt-auto">
            ...
            <p className="text-[12px] text-foreground/40 mt-0.5 leading-tight truncate">Ofertado</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-lg sm:rounded-xl border border-border bg-card py-2 sm:py-3 px-3 sm:px-4 text-[12px] sm:text-[13px] md:text-[14px]">
      ...
      <div className="flex gap-1.5 sm:gap-2">
        <button 
          type="button"
          onClick={handlePrint}
          className="flex-1 rounded-full border border-border bg-secondary text-foreground py-1.5 sm:py-2 text-[12px] sm:text-[14px] md:text-[15px] font-bold tracking-wide hover:opacity-90 transition-all"
        >
          Imprimir
        </button>
        {hasCalculated && (
    ...
    <div className="space-y-2 sm:space-y-3 px-2 sm:px-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 sm:gap-2">
        <div className="rounded-lg sm:rounded-xl border border-green-200 bg-green-50 p-2 sm:p-3">
          <p className="text-[12px] sm:text-[12px] font-semibold uppercase tracking-wider text-green-600 mb-1">Carta atualizada</p>
          <p className="text-base sm:text-lg font-bold text-green-900 break-words">{money(result.c)}</p>
          <p className="text-[12px] sm:text-[12px] text-green-700 mt-1">base simples da carta</p>
        </div>
        <div className="rounded-lg sm:rounded-xl border border-yellow-200 bg-yellow-50 p-2 sm:p-3">
          <p className="text-[12px] sm:text-[12px] font-semibold uppercase tracking-wider text-yellow-600 mb-1">Taxa adm. em R$</p>
          <p className="text-base sm:text-lg font-bold text-yellow-900 break-words">{money(result.adminValue)}</p>
          <p className="text-[12px] sm:text-[12px] text-yellow-700 mt-1">{pct(result.a)} sobre a carta</p>
        </div>
        <div className="rounded-lg sm:rounded-xl border border-red-200 bg-red-50 p-2 sm:p-3">
          <p className="text-[12px] sm:text-[12px] font-semibold uppercase tracking-wider text-red-600 mb-1">Categoria simplificada</p>
          <p className="text-base sm:text-lg font-bold text-red-900 break-words">{money(result.categoryBase)}</p>
          <p className="text-[12px] sm:text-[12px] text-red-700 mt-1">equivale a {pct(result.categoryBasePct)} da carta</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 sm:gap-2">
        <div className="rounded-lg sm:rounded-xl border border-green-300 bg-green-50 p-2.5 sm:p-4">
          <span className="inline-flex items-center rounded-full bg-green-100 px-2 py-0.5 text-[12px] sm:text-[12px] font-bold text-green-700 mb-1.5 sm:mb-2">
            ✓ Mais simples
          </span>
          <h3 className="font-bold text-[14px] md:text-[15px] sm:text-[14px] md:text-[15px] mb-1 sm:mb-2">Lance sobre carta de crédito</h3>
          <p className="text-[12px] sm:text-[12px] text-foreground/70 mb-1.5 sm:mb-2">O percentual incide diretamente sobre a carta atualizada.</p>
          <div className="rounded-lg bg-white p-1.5 sm:p-2 border border-green-200 font-mono text-[12px] sm:text-[12px] mb-1.5 sm:mb-2 overflow-x-auto">
          ...
          <p className="text-[12px] sm:text-[14px] md:text-[15px] font-bold text-green-700 break-words">{money(result.lanceOnCredit)}</p>
        </div>

        <div className="rounded-lg sm:rounded-xl border border-yellow-300 bg-yellow-50 p-2.5 sm:p-4">
          <span className="inline-flex items-center rounded-full bg-yellow-100 px-2 py-0.5 text-[12px] sm:text-[12px] font-bold text-yellow-700 mb-1.5 sm:mb-2">
            ⚠ Base maior
          </span>
          <h3 className="font-bold text-[14px] md:text-[15px] sm:text-[14px] md:text-[15px] mb-1 sm:mb-2">Lance sobre categoria</h3>
          <p className="text-[12px] sm:text-[12px] text-foreground/70 mb-1.5 sm:mb-2">O percentual incide sobre carta + taxa adm.; por isso, o valor exigido sobe.</p>
          <div className="rounded-lg bg-white p-1.5 sm:p-2 border border-yellow-200 font-mono text-[12px] sm:text-[12px] mb-1.5 sm:mb-2 overflow-x-auto">
          ...
          <p className="text-[12px] sm:text-[14px] md:text-[15px] font-bold text-yellow-700 break-words">{money(result.lanceOnCategory)}</p>
        </div>
      </div>

      {/* Impacto */}
      <div className="rounded-lg sm:rounded-xl border-2 border-yellow-400 bg-yellow-50 p-2.5 sm:p-4">
        <p className="text-[12px] sm:text-[12px] font-bold uppercase tracking-wider text-yellow-600 mb-1">Diferença gerada pela base de cálculo</p>
        <p className="text-xl sm:text-2xl font-black text-yellow-900 break-words">{money(result.diff)}</p>
        <p className="text-[12px] sm:text-[13px] md:text-[14px] text-yellow-800 mt-1.5 sm:mt-2">Esse é o valor adicional exigido quando o mesmo lance de {pct(result.l)} é calculado sobre categoria.</p>
      </div>

      {/* Insights */}
      <div className="space-y-1.5 sm:space-y-2">
        <div className="rounded-lg border-l-4 border-green-500 bg-green-50 p-2 sm:p-3">
          <p className="text-[12px] sm:text-[13px] md:text-[14px] font-bold text-green-700 mb-0.5 sm:mb-1">✓ Leitura prática:</p>
          <p className="text-[12px] sm:text-[12px] text-green-800">quando o contrato usa lance sobre carta, o cliente sabe exatamente quanto precisa ofertar em relação ao crédito contratado.</p>
        </div>
        <div className="rounded-lg border-l-4 border-yellow-500 bg-yellow-50 p-2 sm:p-3">
          <p className="text-[12px] sm:text-[13px] md:text-[14px] font-bold text-yellow-700 mb-0.5 sm:mb-1">⚠ Atenção:</p>
          <p className="text-[12px] sm:text-[12px] text-yellow-800">quando a base é categoria, o mesmo percentual pode exigir mais dinheiro do cliente. No exemplo, 30% deixa de ser {money(result.lanceOnCredit)} e passa a ser {money(result.lanceOnCategory)}.</p>
        </div>
        <div className="rounded-lg border-l-4 border-red-500 bg-red-50 p-2 sm:p-3">
          <p className="text-[12px] sm:text-[13px] md:text-[14px] font-bold text-red-700 mb-0.5 sm:mb-1">🔒 Proteção técnica:</p>
          <p className="text-[12px] sm:text-[12px] text-red-800">a regra válida é sempre a do contrato e do regulamento do grupo. Este simulador apenas compara a matemática das bases de cálculo.</p>
        </div>
      </div>
      ...
      <div className="rounded-lg sm:rounded-lg border border-border overflow-hidden">
        <div className="bg-card px-2 sm:px-3 py-1.5 sm:py-2 border-b border-border">
          <p className="text-[12px] sm:text-[14px] md:text-[15px] font-bold uppercase tracking-wider">Memória de cálculo</p>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-[12px] sm:text-[12px]">
            ...
            <tbody>
              {lastRows.map((r, i) => (
                <tr key={i} className={i === 1 ? "bg-yellow-50" : "bg-white"}>
                  <td className="px-1 sm:px-2 py-1 sm:py-1.5 text-left font-medium text-[12px] sm:text-[12px]">{r.modalidade}</td>
