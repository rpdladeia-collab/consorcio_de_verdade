...
export default function SimuladorLanceCartaXCategoria() {
  ...
  const formPanel = (
    <form onSubmit={(e) => { e.preventDefault(); handleCalculate(); }} className="space-y-2 sm:space-y-3">
      <div>
        <label className="block text-[11px] sm:text-xs font-medium text-foreground/60 mb-1">
          Valor da carta atualizada (R$)
        </label>
        <input 
          type="number" 
        ...
        />
        <p className="text-[9px] sm:text-[10px] text-foreground/40 mt-0.5">É o valor da carta de crédito usada como referência no momento da simulação.</p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-1.5 sm:gap-2">
        <div>
          ...
          />
          <p className="text-[9px] sm:text-[10px] text-foreground/40 mt-0.5">Percentual sobre a carta</p>
        </div>
        <div>
          <label className="block text-[11px] sm:text-xs font-medium text-foreground/60 mb-1">
            % de lance (%)
          ...
          <input 
            ...
            className="input w-full text-sm sm:text-base"
            value={lanceRate} 
            onChange={(e) => setLanceRate(e.target.value)} 
          />
          <p className="text-[9px] sm:text-[10px] text-foreground/40 mt-0.5">Percentual ofertado</p>
        </div>
      </div>

      <button 
        type="submit"
        className="w-full rounded-full bg-[var(--orange)] text-white py-2 sm:py-2.5 text-[11px] sm:text-xs font-bold tracking-wide hover:opacity-90 active:scale-[0.98] transition-all"
      >
        Analisar diferença
      </button>
      ...
      <div className="flex gap-1.5 sm:gap-2">
        <button 
          type="button"
          onClick={handlePrint}
          className="flex-1 rounded-full border border-border bg-secondary text-foreground py-1.5 sm:py-2 text-[10px] sm:text-xs font-bold tracking-wide hover:opacity-90 transition-all"
        >
          Imprimir
        </button>
        {hasCalculated && (
    ...
    <div className="space-y-2 sm:space-y-3 px-2 sm:px-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 sm:gap-2">
        <div className="rounded-lg sm:rounded-xl border border-green-300 bg-green-50 p-2.5 sm:p-4">
          ...
          <h3 className="font-bold text-xs sm:text-sm mb-1 sm:mb-2">Lance sobre carta de crédito</h3>
          ...
          <p className="text-[9px] sm:text-xs font-bold text-green-700 break-words">{money(result.lanceOnCredit)}</p>
        </div>
        ...
        <div className="rounded-lg sm:rounded-xl border border-yellow-300 bg-yellow-50 p-2.5 sm:p-4">
          ...
          <h3 className="font-bold text-xs sm:text-sm mb-1 sm:mb-2">Lance sobre categoria</h3>
          ...
          <p className="text-[9px] sm:text-xs font-bold text-yellow-700 break-words">{money(result.lanceOnCategory)}</p>
        </div>
      </div>
      ...
      <div className="rounded-lg sm:rounded-xl border-2 border-yellow-400 bg-yellow-50 p-2.5 sm:p-4">
        ...
        <p className="text-[9px] sm:text-[11px] text-yellow-800 mt-1.5 sm:mt-2">Esse é o valor adicional exigido quando o mesmo lance de {pct(result.l)} é calculado sobre categoria.</p>
      </div>

      {/* Insights */}
      <div className="space-y-1.5 sm:space-y-2">
        <div className="rounded-lg border-l-4 border-green-500 bg-green-50 p-2 sm:p-3">
          <p className="text-[10px] sm:text-[11px] font-bold text-green-700 mb-0.5 sm:mb-1">✓ Leitura prática:</p>
        ...
        <div className="rounded-lg border-l-4 border-yellow-500 bg-yellow-50 p-2 sm:p-3">
          <p className="text-[10px] sm:text-[11px] font-bold text-yellow-700 mb-0.5 sm:mb-1">⚠ Atenção:</p>
        ...
        <div className="rounded-lg border-l-4 border-red-500 bg-red-50 p-2 sm:p-3">
          <p className="text-[10px] sm:text-[11px] font-bold text-red-700 mb-0.5 sm:mb-1">🔒 Proteção técnica:</p>
      ...
      <div className="rounded-lg sm:rounded-lg border border-border overflow-hidden">
        <div className="bg-card px-2 sm:px-3 py-1.5 sm:py-2 border-b border-border">
          <p className="text-[10px] sm:text-xs font-bold uppercase tracking-wider">Memória de cálculo</p>
        </div>
