        ...
        <div className="grid lg:grid-cols-[1fr_1.1fr] gap-6 lg:gap-8 items-start">
          <div className="lg:sticky lg:top-20 px-2 sm:px-0">
            ...
            <div className="rounded-xl border border-border bg-card p-2 sm:p-4 space-y-4">
              ...
              <div>
                <label className="text-[11px] sm:text-xs font-medium text-foreground/70 mb-1">Lance livre ofertado (% da carta)</label>
                <input inputMode="decimal" value={bidPct} onChange={(e) => setBidPct(e.target.value)} className="w-full rounded-lg border border-border bg-background px-3 py-2 text-base data-num font-semibold focus:outline-none focus:border-[var(--orange)] transition-colors" />
                <input type="range" min={0} max={80} step={1} value={parseFloat(bidPct) || 0} onChange={(e) => setBidPct(e.target.value)} className="w-full mt-2 accent-[var(--orange)]" />
              </div>
              <Field label="Lance médio histórico de referência (%)" value={referenceBidPct} onChange={setReferenceBidPct} />
              <button onClick={runAudit} disabled={calc.isPending} className="w-full inline-flex items-center justify-center gap-2 rounded-full bg-[var(--orange)] text-white px-4 py-2 sm:py-2.5 text-[11px] sm:text-sm font-semibold transition-transform hover:scale-[1.01] active:scale-[0.98] disabled:opacity-60">
              ...
              <Collapsible title="Modo avançado" subtitle="Taxa, prazo, parcelas pagas e destino do lance" open={advancedOpen} onToggle={() => setAdvancedOpen((o) => !o)}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                  ...
                  <div className="sm:col-span-2">
                    <label className="text-[11px] sm:text-xs font-medium text-foreground/70 mb-1">Destino do lance ao contemplar</label>
                    <div className="flex gap-1.5">
                      {(["abater_parcela", "reduzir_prazo"] as const).map((m) => (
                        <button key={m} onClick={() => setLanceUse(m)} className={`flex-1 rounded-lg border px-2 py-2 text-[11px] sm:text-sm font-medium transition-colors ${lanceUse === m ? "border-[var(--orange)] text-[var(--orange)] bg-[color-mix(in_oklch,var(--orange)_8%,transparent)]" : "border-border text-foreground/60"}`}>
          ...
          <div id="resultado" className="scroll-mt-24">
            {!result && !calc.isPending && (
              <div className="rounded-xl border border-dashed border-border bg-secondary/40 p-3 sm:p-6 text-center">
                <Gauge className="w-8 h-8 text-foreground/30 mx-auto mb-3" />
                <p className="font-semibold text-sm sm:text-base">Sua análise aparece aqui</p>
                <p className="text-[10px] sm:text-xs text-foreground/55 mt-1 max-w-sm mx-auto">Preencha os dados e clique em analisar.</p>
              </div>
            )}
            {calc.isPending && (
              <div className="rounded-xl border border-border bg-card p-3 sm:p-6 text-center">
                ...
                <p className="text-foreground/60 text-xs sm:text-sm">Processando no servidor…</p>
              </div>
            )}
            {result && (
              <div className="space-y-4 animate-[fadeIn_0.4s_ease-out] px-2 sm:px-0">
                {result.warnings.length > 0 && (
                  <div className="rounded-lg border border-[color-mix(in_oklch,var(--orange)_35%,transparent)] bg-[color-mix(in_oklch,var(--orange)_10%,transparent)] p-2 sm:p-3 space-y-1">
                    {result.warnings.map((w, i) => (<p key={i} className="text-[10px] sm:text-xs text-foreground/75">{w}</p>))}
                  </div>
                )}
                ...
                <DiagnosticCard
                  ...
                  narrative={<p className="text-[10px] sm:text-xs">{result.decisionText}</p>}
                />

                <MeaningBlock label="Lance Livre" textClass="text-[10px] sm:text-xs">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="inline-flex items-center gap-1 text-[10px] sm:text-xs text-foreground/45">
                ...
                <Collapsible title="Sensibilidade e memória de cálculo" subtitle="Como diferentes lances se comparam à referência" open={true} onToggle={() => {}}>
                  <div className="space-y-4 pt-2">
                    <div className="overflow-x-auto">
                      <table className="w-full text-[8px] sm:text-xs border-collapse min-w-[400px]">
                        <thead>
                          <tr className="text-left text-foreground/50 text-[7px] sm:text-xs uppercase tracking-wide">
...
function Field({ label, value, onChange, big }: { label: string; value: string; onChange: (v: string) => void; big?: boolean }) {
  return (
    <div>
      <label className="text-[11px] sm:text-xs font-medium text-foreground/70 mb-1">{label}</label>
