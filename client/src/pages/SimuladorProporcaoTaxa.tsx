...
export default function SimuladorProporcaoTaxa() {
  ...
  const totalPaidAtMonth = (ownNum + fgtsNum) + (selectedMonth * (currentCreditAtMonth * (1 + adminPctNum / 100) / termNum));
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <label className="block col-span-2">
          <span className="text-xs font-bold uppercase text-foreground/60">Carta de crédito (R$)</span>
          <input type="number" className="input mt-1 w-full" value={form.credit} onChange={(e) => set("credit", e.target.value)} />
        </label>
        <label className="block">
          <span className="text-xs font-bold uppercase text-foreground/60">Taxa adm. (%)</span>
          <input type="number" className="input mt-1 w-full" value={form.adminPct} onChange={(e) => set("adminPct", e.target.value)} />
        </label>
        <label className="block">
          <span className="text-xs font-bold uppercase text-foreground/60">Parcelas pagas</span>
          <input type="number" className="input mt-1 w-full" value={form.paid} onChange={(e) => set("paid", e.target.value)} />
        </label>
        <label className="block">
          <span className="text-xs font-bold uppercase text-foreground/60">Lance próprio (R$)</span>
          <input type="number" className="input mt-1 w-full" value={form.own} onChange={(e) => set("own", e.target.value)} />
        </label>
        <label className="block">
          <span className="text-xs font-bold uppercase text-foreground/60">FGTS (R$)</span>
          <input type="number" className="input mt-1 w-full" value={form.fgts} onChange={(e) => set("fgts", e.target.value)} />
        </label>
        <label className="block col-span-2">
          <span className="text-xs font-bold uppercase text-foreground/60">Lance embutido (R$)</span>
          <input type="number" className="input mt-1 w-full" value={form.embedded} onChange={(e) => set("embedded", e.target.value)} />
        </label>
        <label className="block">
          <span className="text-xs font-bold uppercase text-foreground/60">Total de parcelas</span>
          <input type="number" className="input mt-1 w-full" value={form.totalParcelas} onChange={(e) => set("totalParcelas", e.target.value)} />
        </label>
        <label className="block">
          <span className="text-xs font-bold uppercase text-foreground/60">Correção Anual (%)</span>
          <input type="number" className="input mt-1 w-full" value={form.correcaoAnualPct} onChange={(e) => set("correcaoAnualPct", e.target.value)} />
        </label>
      </div>
      <button onClick={() => setHasCalculated(true)} className="w-full btn-primary py-3">Calcular proporção da taxa</button>
    </div>
  );
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="bg-red-50 border border-red-100 rounded-xl p-4 flex gap-3">
          ...
          <div>
            <p className="text-xs font-bold text-red-900 uppercase">Alerta de eficiência</p>
            <p className="text-xs text-red-800 mt-1">A eficiência projetada caiu ao longo do prazo. Isso significa que o custo da operação aumenta proporcionalmente quando comparado ao dinheiro novo utilizado.</p>
          </div>
        </div>
          ...
          <div className="bg-[#FAF5EA] w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden border border-[#DDD6C8] animate-in zoom-in-95 duration-200">
            <div className="bg-[#0A0A08] p-6 text-white flex justify-between items-center">
              <div>
                ...
                <p className="text-[#C9C4B8] text-xs mt-1 font-['IBM_Plex_Mono'] uppercase tracking-widest">Entenda a Eficiência do seu Crédito</p>
              </div>
            ...
            <div className="p-8 space-y-6 overflow-y-auto max-h-[70vh]">
              <section>
                <h4 className="text-[#0A0A08] font-bold text-sm uppercase mb-2 flex items-center gap-2">
                  <div className="w-1.5 h-4 bg-[#FF4E1F]"></div>
                  O Conceito de "Dinheiro Novo"
                </h4>
                <p className="text-[#726C60] text-sm leading-relaxed">
              ...
              <section className="bg-white/50 border border-[#DDD6C8] rounded-xl p-4">
                <h4 className="text-[#0A0A08] font-bold text-xs uppercase mb-3">Como a eficiência é medida:</h4>
              ...
              <div className="pt-4 border-t border-[#DDD6C8] flex justify-end">
                <button 
                  ...
                  className="px-6 py-2 bg-[#0A0A08] text-white text-xs font-bold uppercase tracking-widest rounded-lg hover:bg-[#FF4E1F] transition-colors"
                >
                  Entendi o Racional
                </button>
              </div>
