...
export default function SimuladorVendaCartaContemplada() {
  ...
  const formPanel = (
    <form onSubmit={(e) => { e.preventDefault(); setHasCalculated(true); }} className="space-y-3">
      <div className="text-sm font-bold text-foreground/80 mb-2 uppercase tracking-wider">Parâmetros</div>
      <div>
        <label className="block text-[11px] font-medium text-foreground/60 mb-1">1) Carta de crédito inicial (R$)</label>
        <input type="number" className="input w-full" value={valorInicial} onChange={(e) => setValorInicial(e.target.value)} />
      </div>
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="block text-[11px] font-medium text-foreground/60 mb-1">2) Parcela inicial (R$)</label>
          <input type="number" className="input w-full" value={parcelaInicial} onChange={(e) => setParcelaInicial(e.target.value)} />
        </div>
        <div>
          <label className="block text-[11px] font-medium text-foreground/60 mb-1">3) Prazo total (meses)</label>
          <input type="number" className="input w-full" value={prazoTotal} onChange={(e) => setPrazoTotal(e.target.value)} />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="block text-[11px] font-medium text-foreground/60 mb-1">4) Taxa de correção (anual %)</label>
          <input type="number" className="input w-full" value={indiceAnual} onChange={(e) => setIndiceAnual(e.target.value)} />
        </div>
        <div>
          <label className="block text-[11px] font-medium text-foreground/60 mb-1">5) Parcela da contemplação (mês)</label>
          <input type="number" className="input w-full" value={parcelaContemplacao} onChange={(e) => setParcelaContemplacao(e.target.value)} />
        </div>
      </div>
      <div>
        <label className="block text-[11px] font-medium text-foreground/60 mb-1">6) Taxa de repasse (mensal)</label>
        <input type="number" className="input w-full" value={taxaRepasseMensal} onChange={(e) => setTaxaRepasseMensal(e.target.value)} />
      </div>
      <div>
        <label className="block text-[11px] font-medium text-foreground/60 mb-1">7) Lance embutido (R$)</label>
        <input type="number" className="input w-full" value={lanceEmbutido} onChange={(e) => setLanceEmbutido(e.target.value)} />
      </div>
      <div>
        <label className="block text-[11px] font-medium text-foreground/60 mb-1">8) Lance Realizado (R$)</label>
        <input type="number" className="input w-full" value={lanceRealizado} onChange={(e) => setLanceRealizado(e.target.value)} />
      </div>
      <div className="flex gap-2 pt-2">
        <button type="submit" className="flex-1 rounded-lg bg-[var(--orange)] text-white py-2.5 text-xs font-bold uppercase tracking-widest hover:opacity-90 transition-all">Calcular</button>
        <button type="button" onClick={() => setHasCalculated(false)} className="flex-1 rounded-lg bg-gray-200 text-foreground py-2.5 text-xs font-bold uppercase tracking-widest hover:bg-gray-300 transition-all">Reset</button>
      </div>
  ...
  const resultsPanel = (
    <div className="space-y-4">
      <div className="text-sm font-bold text-foreground/80 mb-2 uppercase tracking-wider">Projeção de venda - carta contemplada</div>
      {hasCalculated ? (
        <div className="space-y-4">
          ...
          <button onClick={handlePdf} disabled={pdfLoading} className="w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-white border border-border font-bold text-xs uppercase tracking-widest hover:bg-gray-50 transition-all shadow-sm">
