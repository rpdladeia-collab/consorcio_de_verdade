...
for (const mediaPath of mediaFiles) {
  ...
  const tracked = trackedFiles.has(repoPath);
...
for (const reference of references.filter((item) => item.resolvedFile)) {
  const tracked = trackedFiles.has(reference.resolvedFile);
...
if (verifyManifest) {
  ...
  const manifestIsTracked = gitTrackedFiles([manifestRepoPath]).has(manifestRepoPath);
