...
export function runCancelamento(input: CancelamentoInput) {
  ...
  // --- Custo de oportunidade (Capitalização dos desembolsos) ---
  const cdiMonthly = Math.pow(1 + cdiAnnualPct/100, 1/12) - 1;
  
  // Cálculo do que o dinheiro renderia se investido em vez de pago ao consórcio
  let montanteInvestido = 0;
  monthlyData.forEach(row => {
    // Para cada parcela paga, ela rende CDI do mês em que foi paga até o mês do cancelamento
    const mesesRendimento = canceledMonth - row.mes;
    montanteInvestido += row.parcela * Math.pow(1 + cdiMonthly, mesesRendimento);
  });

  // Prejuízo Indireto = O que eu teria no bolso se tivesse investido - O que eu realmente paguei
  const prejuizoIndireto = Math.max(0, montanteInvestido - totalPaidByClient);
  
  // --- Resumo ---
  const prejuizoDireto = totalPaidByClient - baseAposMulta;
  const perdaPatrimonialTotal = prejuizoDireto + prejuizoIndireto;
  const prejuizoTotalPct = totalPaidByClient > 0 ? (perdaPatrimonialTotal / totalPaidByClient) * 100 : 0;
