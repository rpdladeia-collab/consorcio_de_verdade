...
import { calcDegradacaoProgressiva, DegradacaoResult } from './proporcaoTaxa';
...
export interface EficienciaTaxaTableRow {
  indicator: string;
  value: string;
  reading: string;
}

export interface EficienciaTaxaReadbox {
  title: string;
  body: string;
  formula?: string;
}

export interface EficienciaTaxaResult {
  nominal: number;           // Taxa nominal (%)
  onLiquid: number;          // Taxa sobre carta líquida (%)
  onNew: number;             // Taxa sobre dinheiro novo (%)
  penalty: number;           // Penalidade (%)
  adminValue: number;        // Valor da taxa em R$
  liquidCredit: number;      // Carta líquida
  newMoney: number;          // Dinheiro novo
  paidAmount: number;        // Parcelas pagas + recursos próprios
  credit: number;            // Crédito inicial (carta cheia)
  embedded: number;          // Lance embutido
  table?: EficienciaTaxaTableRow[]; // Tabela de indicadores
  readboxes?: EficienciaTaxaReadbox[]; // Explicacao do racional
  degradacao?: DegradacaoResult; // Degradacao progressiva
}
...
export interface EstruturaResult {
  ...
  eficienciaTaxa?: EficienciaTaxaResult; // Eficiência da taxa
}
...
export function simulateEstruturaDoPlano(opts: EstruturaOptions): EstruturaResult {
  ...
  if (opts.lanceProprio || opts.lanceFgts || opts.lanceEmbutido) {
    const scenario: ContemplationScenario = {
      ...
      strategy: opts.estrategiaPos === 'reduzir_prazo' ? 'term' : 'payment',
  ...
  // Cálculo de Eficiência da Taxa (usando racional correto do Proporção da Taxa)
  // Taxa de administração em R$ (sobre crédito inicial)
  const adminValue = c.credit * c.adminRate / 100;
  // Crédito líquido = crédito inicial - embutido
  const liquidCredit = Math.max(0, c.credit - (opts.lanceEmbutido || 0));
  // Parcelas pagas em R$ (valor médio de parcela × quantidade)
  const avgPayment = proposal.rows.length > 0 
    ? proposal.rows.reduce((sum: number, r: any) => sum + r.payment, 0) / proposal.rows.length 
    : 0;
  const paidAmount = (opts.parcelasPagas || 0) * avgPayment;
  // Dinheiro novo = crédito inicial - embutido - lance próprio - FGTS - parcelas pagas
  const newMoney = Math.max(0, c.credit - (opts.lanceEmbutido || 0) - (opts.lanceProprio || 0) - (opts.lanceFgts || 0) - paidAmount);
  // Taxas em percentual
  const nominal = c.adminRate;
  ...
  const penalty = Number.isFinite(onNew) && nominal > 0 ? (onNew / nominal - 1) * 100 : Infinity;

  let eficienciaTaxa: EficienciaTaxaResult | undefined = undefined;
  if (c.adminRate > 0) {
    const degradacao = calcDegradacaoProgressiva(
      c.credit,
      c.adminRate,
      opts.lanceProprio || 0,
      opts.lanceFgts || 0,
      opts.lanceEmbutido || 0,
      c.term
    );
    // Tabela de indicadores (7 linhas)
    const table: EficienciaTaxaTableRow[] = [
      { indicator: 'Taxa adm. em R$', value: `R$ ${adminValue.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, reading: 'Taxa nominal multiplicada pela carta cheia.' },
      { indicator: 'Carta cheia', value: `R$ ${c.credit.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, reading: 'Base comercial do contrato.' },
      { indicator: 'Lance embutido', value: `R$ ${(opts.lanceEmbutido || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, reading: 'Reduz crédito disponível; não é dinheiro novo.' },
      { indicator: 'Carta líquida após embutido', value: `R$ ${liquidCredit.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, reading: 'Crédito disponível para compra.' },
      { indicator: 'Parcelas pagas + recursos próprios', value: `R$ ${paidAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, reading: 'Desembolsos que reduzem a leitura de dinheiro novo.' },
      { indicator: 'Dinheiro novo efetivo', value: `R$ ${newMoney.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, reading: 'Base econômica mais rigorosa.' },
      { indicator: 'Taxa sobre dinheiro novo', value: `${onNew.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}%`, reading: 'Indicador principal de eficiência.' },
    ];

    // Readboxes com explicação do racional
    const readboxes: EficienciaTaxaReadbox[] = [
      {
        title: 'Base correta',
      ...
      },
      {
        ...
        formula: `R$ ${c.credit.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} − R$ ${(opts.lanceEmbutido || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} − R$ ${(opts.lanceProprio || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} − R$ ${(opts.lanceFgts || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} − R$ ${paidAmount.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} = R$ ${newMoney.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
      },
    ];
