...
function parseRanges(text: string, term: number) {
  ...
  lines.forEach((line, idx) => {
    let sanitizedLine = line.replace(/\s+/g, "").replace(/[–—]/g, "-");
    const cleaned = sanitizedLine.replace(/m[eê]s(es)?/ig, ").replace(/R\$/ig, ");
    const m =
      cleaned.match(/(\d+)\s*(?:-|a|até|ate)\s*(\d+)\s*[:=]\s*([\d.,]+)/i) ||
      cleaned.match(/(\d+)\s*[:=]\s*([\d.,]+)/i);
    ...
    if (m.length === 4) {
      ...
      value = parseFloat(m[3].replace(/\./g, "").replace(",", "."));
    } else {
      start = parseInt(m[1], 10);
      end = start;
      value = parseFloat(m[2].replace(/\./g, "").replace(",", "."));
    }
    if (isNaN(value)) value = 0; // Fallback para valor seguro
...
// Formata valor monetário para mensagens de warning (sem depender de DOM)
...
export function buildSchedule(opts: ScheduleOptions): ScheduleResult {
  ...
  const initialObligation = fc + ta + fr;
  let correctionNominal = 0, fcCorrection = 0, adminCorrection = 0, reserveCorrection = 0;
  let insuranceTotal = 0, paidTotal = 0, overpay = 0;
  ...
  for (let m = 1; m <= term; m++) {
    ...
    if (mode === 'nonlinear' && parsed.map.has(m)) {
      desiredComponents = (parsed.map.get(m)!) * correctionFactor;
    }

    desiredComponents = Math.max(0, desiredComponents);
    const outstanding = fc + ta + fr;
    const componentPay = Math.min(desiredComponents, outstanding);
  ...
  if (mode === 'nonlinear') {
    ...
    if (missing.length)
      warnings.push(
        `${missing.length} mês(es) sem faixa não linear: nesses meses foi usada a parcela linear estimada, sempre com as correções do índice aplicadas.`
      );
    if (residual > 1)
      warnings.push(
        `Após aplicar as correções do índice, ainda existe saldo residual estimado de ${moneyStr(residual)}. Revise as faixas informadas ou a regra contratual de recálculo da administradora.`
      );
    if (overpay > 1)
      warnings.push(
        `As parcelas informadas excedem a obrigação contratual em ${moneyStr(overpay)}. O motor limitou o pagamento ao saldo restante.`
      );
  }
