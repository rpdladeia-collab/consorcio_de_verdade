...
import { analyzeTechnicalReport } from '../lib/financialAnalysis';
...
export const raioxRouter = router({
  ...
  simulePlano: publicProcedure
    ...
    .mutation(({ input }) => {
      ...
      const analysis = analyzeTechnicalReport(schedule, input.adminRate, input.adjRate, Number(input.adjEvery), input.mode, input.insuranceRate);
      ...
      return { schedule, analysis, simulationId, generatedAt };
    }),

  /**
   * Análise Financeira Automática
   */
  financialAnalysis: publicProcedure
    .input(
      z.object({
        credit: z.number().min(0),
        term: z.number().int().min(1).max(240),
